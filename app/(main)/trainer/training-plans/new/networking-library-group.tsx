...
export function NetworkingLibraryGroup({ networkingTopicIdSet, selectedIds, children }: Props) {
  const [open, setOpen] = useState(false);

  const hasNetworkingInPlan = useMemo(() => {
    for (const id of selectedIds) {
      if (planEntryBelongsToSectionTopic(id, networkingTopicIdSet)) return true;
    }
    return false;
  }, [selectedIds, networkingTopicIdSet]);

  const shellClass = hasNetworkingInPlan ? PORTAL_NESTED_ACTIVE : PORTAL_NESTED_INACTIVE;
    ...
    <div className={`${shellClass} flex min-h-0 flex-col`}>
      ...
      >
        <div className="min-w-0">
          <h2 className="text-lg font-semibold tracking-wide text-slate-900">Networking</h2>
