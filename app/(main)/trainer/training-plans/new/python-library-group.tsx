...
export function PythonLibraryGroup({ pythonTopicIdSet, selectedIds, children }: Props) {
  const [open, setOpen] = useState(false);

  const hasPythonInPlan = useMemo(() => {
    for (const id of selectedIds) {
      if (planEntryBelongsToSectionTopic(id, pythonTopicIdSet)) return true;
    }
    return false;
  }, [selectedIds, pythonTopicIdSet]);

  const shellClass = hasPythonInPlan ? PORTAL_NESTED_ACTIVE : PORTAL_NESTED_INACTIVE;
    ...
    <div className={`${shellClass} flex min-h-0 flex-col`}>
      ...
      >
        <div className="min-w-0">
          <h2 className="text-lg font-semibold tracking-wide text-slate-900">Python</h2>
