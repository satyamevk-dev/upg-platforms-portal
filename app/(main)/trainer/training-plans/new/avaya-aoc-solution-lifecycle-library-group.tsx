...
export function AvayaAocSolutionLifecycleLibraryGroup({
  ...
  const hasLifecycleInPlan = useMemo(() => {
    for (const id of selectedIds) {
      if (planEntryBelongsToSectionTopic(id, solutionLifecycleTopicIdSet)) return true;
    }
    return false;
  }, [selectedIds, solutionLifecycleTopicIdSet]);

  const shellClass = hasLifecycleInPlan ? PORTAL_NESTED_ACTIVE : PORTAL_NESTED_INACTIVE;
