...
export function LinuxLibraryGroup({ linuxTopicIdSet, selectedIds, children }: Props) {
  const [open, setOpen] = useState(false);

  const hasLinuxInPlan = useMemo(() => {
    for (const id of selectedIds) {
      if (planEntryBelongsToSectionTopic(id, linuxTopicIdSet)) return true;
    }
    return false;
  }, [selectedIds, linuxTopicIdSet]);

  const shellClass = hasLinuxInPlan ? PORTAL_NESTED_ACTIVE : PORTAL_NESTED_INACTIVE;
    ...
    <div className={`${shellClass} flex min-h-0 flex-col`}>
      ...
      >
        <div className="min-w-0">
          <h2 className="text-lg font-semibold tracking-wide text-slate-900">LINUX</h2>
