...
export function AvayaAocLibraryGroup({ avayaAocTopicIdSet, selectedIds, children }: Props) {
  const [open, setOpen] = useState(false);

  const hasAocInPlan = useMemo(() => {
    for (const id of selectedIds) {
      if (planEntryBelongsToSectionTopic(id, avayaAocTopicIdSet)) return true;
    }
    return false;
  }, [selectedIds, avayaAocTopicIdSet]);

  const shellClass = hasAocInPlan ? PORTAL_NESTED_ACTIVE : PORTAL_NESTED_INACTIVE;
