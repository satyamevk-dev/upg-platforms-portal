...
const levelAccent: Record<string, string> = {
  Basic: "from-[#00786f]/15 to-[#00A89E]/5 ring-[#00A89E]/20",
  Intermediate: "from-[#0d9488]/12 to-[#14b8a6]/5 ring-teal-500/15",
...
export function TraineeCourseSyllabusSection({ title, syllabusFilePath, body, sections }: Props) {
  ...
  return (
    <div className="border-t border-[#d8d0c4]/90 bg-gradient-to-b from-slate-50/40 to-white/30 px-6 py-8 sm:px-10 sm:py-10">
      <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#00786f]">Reference</p>
      ...
      <div className="mt-8 space-y-8">
        {sections.map((sec) => {
          ...
          return (
            ...
            >
              <div
                className={`border-b border-[#d8d0c4] bg-gradient-to-r px-5 py-4 sm:px-6 ${accent} ring-1 ring-inset`}
              >
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[36rem] border-collapse text-left text-sm">
                  <thead>
                    <tr className="border-b border-[#d8d0c4] bg-[#faf9f7]">
                  ...
                  <tbody>
                    {sec.modules.map((row, idx) => (
                      <tr
                        key={row.id}
                        className={`border-b border-[#d8d0c4] transition-colors last:border-b-0 ${
                          row.inPlan ? "bg-[#f0faf8]/95 hover:bg-[#e8f7f4]" : "bg-white hover:bg-slate-50/80"
                        }`}
                      >
                        ...
                        <td className="whitespace-nowrap px-4 py-3">
                          {row.inPlan ? (
                            <span className="inline-flex items-center gap-1 rounded-full bg-[#00A89E]/12 px-2.5 py-1 text-xs font-semibold text-[#006b64] ring-1 ring-[#00A89E]/25">
                              <span className="size-1.5 rounded-full bg-[#00A89E]" aria-hidden />
                              In your plan
                            </span>
                          ) : (
